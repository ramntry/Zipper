...
int ZStream::DeflateInit(int level, Place const &p)
{
	...
	return check(deflateInit(this, level)
			...
			, Z_VERSION_ERROR, Z_E(Z_VERSION_ERROR)" the version of zlib.h and the version of the library linked do not match"
			, Z_OK);
}
...
int ZStream::Deflate(int flush, Place const &p)
{
	...
	return check(deflate(this, flush)
			...
			, Z_STREAM_ERROR, Z_E(Z_STREAM_ERROR)" the stream state is inconsistent (for example if next_in or next_out is NULL)"
...
int ZStream::Inflate(int flush, Place const &p)
{
	...
	return check(inflate(this, flush)
			...
			, Z_DATA_ERROR, Z_E(Z_DATA_ERROR)" input data was corrupted (input stream not conforming to the zlib format or incorrect check value)"
			, Z_STREAM_ERROR, Z_E(Z_STREAM_ERROR)" the stream structure was inconsistent (for example if next_in or next_out was NULL)"
			, Z_MEM_ERROR, Z_E(Z_MEM_ERROR)" there was not enough memory"
			, Z_BUF_ERROR, Z_E(Z_BUF_ERROR)" no progress is possible or if there was not enough room in the output buffer when Z_FINISH is used"
			, Z_OK);
}
